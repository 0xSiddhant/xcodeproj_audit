...
final class DashboardManager {
    ...
    func generateDashboard() throws {
        ...
        let result = generateCodeStats(for: xcodeProj.pbxproj, projectRoot: root)
        print(result)
    }
    
    func generateMeta() throws {
        let metadata = fetchMetadata(from: xcodeProj.pbxproj)
        print(metadata)
    }
    
    func generateCodeStats() throws {
        let result = generateCodeStats(for: xcodeProj.pbxproj, projectRoot: xcodeProj.path!.parent())
        print(result)
    }
    
    private func generateCodeStats(for project: PBXProj, projectRoot: Path, config: DashboardConfig = DashboardConfig()) -> DashboardResult {
